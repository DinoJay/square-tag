...
export default function Grid(props) {
  ...
  return <div className={clsx( className, 'flex flex-col w-full overflow-x-hidden overflow-y-auto')}
    style={style}>
    <div className="flex flex-wrap items-center">{range(0, maxPage).map(d =>
      <button className={ clsx("m-1 rounded-full border-2 border-gray px-2", startPage===d && 'bg-myLightRed') } onClick={() => setStartPage(d)}>{d}</button>)}
    </div>
